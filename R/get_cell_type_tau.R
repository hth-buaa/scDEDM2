...
#'
#' @description
#' Computes a cell-type-specific threshold (tau) for binarizing GRN.
...
#'
#' @returns A named numeric vector where names correspond to cell types and values represent the computed binarization thresholds (tau) for each cell type.
#'
#' @details
...
#' 3. Sets the threshold (tau) to the maximum of this computed value and 0.005, ensuring a lower bound of 0.005.
...
#' interest_cell_type_tau = get_cell_type_tau(interest_cell_type_iGRN)
#' base::saveRDS(interest_cell_type_tau, file = "./3 get iGRN/interest_cell_type_tau.rds")
#' }
get_cell_type_tau = function(interest_cell_type_iGRN)
{
  ### Start.
  t_start = base::Sys.time()
  message("Run: Extracting tau for each cell type ", t_start, ".")
  ...
  interest_cell_type_tau = base::rep(999, base::length(interest_cell_type_iGRN))
  base::names(interest_cell_type_tau) = base::names(interest_cell_type_iGRN)
  message("Calulating.")
  for (cell_type in base::names(interest_cell_type_iGRN)) {
    interest_cell_type_tau[cell_type] = base::max(
  ...
  message("The current working directory has been switched to: ", base::getwd(), ".")
  t_end = base::Sys.time()
  message("Finish: Extracting tau for each cell type ", t_end, ".")
  message("Running time: ")
  base::print(t_end - t_start)
  return(interest_cell_type_tau)
}
